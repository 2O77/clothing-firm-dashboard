...
const loadGeoJsonLayer = async () => {
  try {
    ...
    // GeoJSON verisini haritaya ekleyin, hover özellikleri ekleyin
    L.geoJSON(geoJsonData, {
      ...
      onEachFeature: (feature, layer) => {
        layer.on({
          mouseover: (e) => showPopup(e, feature),
          mouseout: hidePopup
        })
      }
    }).addTo(map.value)
...
// İl üzerine gelindiğinde popup'ı gösterme
const showPopup = (e, feature) => {
  const layer = e.target

  // Popup oluştur ve sağ üstteki x işaretini kaldırarak göster
  layer.bindPopup(feature.properties.name, { closeButton: false }).openPopup()

  // Katmanı belirginleştir
  layer.setStyle({
    weight: 3,
    color: '#666',
    fillOpacity: 0.7
...
// İl üzerinden çıkıldığında popup'ı kapatma ve stili sıfırlama
const hidePopup = (e) => {
  const layer = e.target
